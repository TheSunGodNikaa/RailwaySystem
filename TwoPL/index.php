...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'class') {
    ...
    $_SESSION['active_booking'] = [
        'booking_token' => railwayGenerateBookingToken(),
...
<div class="shell">
                ...
                <div class="manifest-shell" id="manifestPanel" style="<?php echo $selectedTrain ? '' : 'display:none;'; ?>">
                    <div class="manifest-top">
                        ...
                        <div>
                            ...
                            <div class="manifest-note">Under 7 years gets 50% off, age 60 to 79 gets 30%, age 80+ gets 50%, military gets 30%, and disability categories A/B/C get 25%/40%/50%. We always apply the highest single concession for each passenger.</div>
                            <div class="manifest-note">Valid document examples for disability verification: Aadhaar card, disability ID card, birth certificate, medical certificate, voter ID, passport, school ID, or hospital document. Enter the supporting document details directly in the passenger card.</div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function defaultPassenger(index){return{name:'',age:'',gender:'',is_military:'N',is_disabled:'N',disability_category:'',document_type:'',document_id:'',sequence:index+1}}
        function getPassengerCount(){return Number(manifestCount?.value||1)}
        function syncManifestRows(){if(!manifestList)return;const count=getPassengerCount();const current=getManifestData({allowPartial:true});while(current.length<count){current.push(defaultPassenger(current.length))}current.length=count;manifestList.innerHTML=current.map((passenger,index)=>`<div class="manifest-card"><h4>Passenger ${index+1}</h4><div class="manifest-grid"><div><label>Full Name</label><input type="text" data-field="name" data-index="${index}" value="${escapeHtml(passenger.name||'')}" placeholder="Passenger name"></div><div><label>Age</label><input type="number" min="0" max="120" data-field="age" data-index="${index}" value="${escapeHtml(passenger.age||'')}" placeholder="Age"></div><div><label>Gender</label><select data-field="gender" data-index="${index}"><option value="">Select</option><option value="Male" ${(passenger.gender||'')==='Male'?'selected':''}>Male</option><option value="Female" ${(passenger.gender||'')==='Female'?'selected':''}>Female</option><option value="Other" ${(passenger.gender||'')==='Other'?'selected':''}>Other</option></select></div><div><label>Military</label><select data-field="is_military" data-index="${index}"><option value="N" ${(passenger.is_military||'N')==='N'?'selected':''}>No</option><option value="Y" ${(passenger.is_military||'N')==='Y'?'selected':''}>Yes</option></select></div><div><label>Disabled</label><select data-field="is_disabled" data-index="${index}"><option value="N" ${(passenger.is_disabled||'N')==='N'?'selected':''}>No</option><option value="Y" ${(passenger.is_disabled||'N')==='Y'?'selected':''}>Yes</option></select></div><div><label>Disability Category</label><select data-field="disability_category" data-index="${index}"><option value="">None</option><option value="A" ${(passenger.disability_category||'')==='A'?'selected':''}>Category A</option><option value="B" ${(passenger.disability_category||'')==='B'?'selected':''}>Category B</option><option value="C" ${(passenger.disability_category||'')==='C'?'selected':''}>Category C</option></select></div><div><label>Document Type</label><input type="text" data-field="document_type" data-index="${index}" value="${escapeHtml(passenger.document_type||'')}" placeholder="Aadhaar / certificate / ID"></div><div><label>Document ID</label><input type="text" data-field="document_id" data-index="${index}" value="${escapeHtml(passenger.document_id||'')}" placeholder="Document number or note"></div></div></div>`).join('')}
        function getManifestData({allowPartial=false}={}){if(!manifestList)return[];const count=getPassengerCount();const passengers=[];for(let index=0;index<count;index+=1){const nameField=manifestList.querySelector(`[data-field="name"][data-index="${index}"]`);const ageField=manifestList.querySelector(`[data-field="age"][data-index="${index}"]`);const genderField=manifestList.querySelector(`[data-field="gender"][data-index="${index}"]`);const militaryField=manifestList.querySelector(`[data-field="is_military"][data-index="${index}"]`);const disabledField=manifestList.querySelector(`[data-field="is_disabled"][data-index="${index}"]`);const disabilityCategoryField=manifestList.querySelector(`[data-field="disability_category"][data-index="${index}"]`);const documentTypeField=manifestList.querySelector(`[data-field="document_type"][data-index="${index}"]`);const documentIdField=manifestList.querySelector(`[data-field="document_id"][data-index="${index}"]`);const passenger={name:nameField?.value.trim()||'',age:ageField?.value.trim()||'',gender:genderField?.value||'',is_military:militaryField?.value||'N',is_disabled:disabledField?.value||'N',disability_category:disabilityCategoryField?.value||'',document_type:documentTypeField?.value.trim()||'',document_id:documentIdField?.value.trim()||'',sequence:index+1};if(!allowPartial&&(!passenger.name||passenger.age===''||Number(passenger.age)<0||!passenger.gender)){return null}if(!allowPartial&&passenger.is_disabled==='Y'&&(!passenger.disability_category||!passenger.document_type||!passenger.document_id)){return null}passengers.push(passenger)}return passengers}
        function resolveClientDiscount(passenger){const age=Number(passenger.age||0);let discount=0;if(age<7||age>=80){discount=50}else if(age>=60){discount=30}if(passenger.is_military==='Y'&&discount<30){discount=30}const disabilityMap={A:25,B:40,C:50};if(passenger.is_disabled==='Y'&&disabilityMap[passenger.disability_category]&&disabilityMap[passenger.disability_category]>discount){discount=disabilityMap[passenger.disability_category]}return discount}
        function renderManifestSummary(passengers){if(!manifestSummary)return;if(!passengers||!passengers.length||!currentTrain){manifestSummary.style.display='none';selectedPassengersInput.value='[]';return}const baseFareCard=Number(currentTrain.PRICE||0);const pricing=passengers.reduce((acc,passenger)=>{const discount=resolveClientDiscount(passenger);const base=baseFareCard;const savings=Math.round((base*discount/100)/10)*10;acc.base+=base;acc.savings+=savings;return acc},{base:baseFareCard*passengers.length,savings:0});manifestSummary.style.display='grid';manifestSummaryCount.textContent=String(passengers.length);manifestSummaryBase.textContent=formatCurrency(pricing.base);manifestSummarySavings.textContent=formatCurrency(pricing.savings);selectedPassengersInput.value=JSON.stringify(passengers)}
