...
$seat_price = $classCatalog[$compartment]['price'] ?? railwayRoundFare((float) ($train['PRICE'] ?? 0));
...
<head>
    ...
    <style>
        :root { --bg:#eef3f8; --card:#fff; --line:#e2e8f0; --text:#0f172a; --muted:#64748b; --accent:#2563eb; --selected:#10b981; --booked:#cbd5e1; }
        *{box-sizing:border-box;font-family:'Plus Jakarta Sans',sans-serif} body{margin:0;background:var(--bg);padding:40px} .wrap{display:flex;gap:30px;max-width:1180px;margin:0 auto}.coach,.side{background:var(--card);border-radius:24px;box-shadow:0 10px 30px rgba(15,23,42,.08)} .coach{flex:1;padding:30px}.side{width:360px;padding:28px;height:fit-content;position:sticky;top:30px}
        ...
        .bay{display:grid;grid-template-columns:repeat(3,1fr) 40px 1fr;gap:10px;padding-bottom:18px;border-bottom:1px solid var(--line)} .main{display:grid;grid-template-columns:repeat(3,1fr);grid-template-rows:1fr 1fr;gap:8px}.sidecol{display:grid;grid-template-rows:1fr 1fr;gap:8px}.aisle{background:#e5e7eb;border-radius:6px}
        .seat{height:46px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;cursor:pointer;border:1px solid #d1d5db;background:#fff;color:#475569}.seat.booked{background:var(--booked);color:#94a3b8;cursor:not-allowed;border:none}.seat.selected{background:var(--selected);color:#fff;border-color:var(--selected);box-shadow:0 0 12px rgba(16,185,129,.35)}
        .pill{display:inline-flex;padding:8px 12px;border-radius:999px;background:#dbeafe;color:var(--accent);font-size:.8rem;font-weight:800;margin-bottom:16px}.muted{color:var(--muted)} .price{font-size:2rem;font-weight:800;color:var(--accent);margin:16px 0}.btn{width:100%;padding:16px;border:none;border-radius:14px;background:var(--accent);color:#fff;font-weight:800;cursor:pointer}
...
<div class="wrap">
    ...
    <div class="side">
        ...
        <form action="booking_2pl.php" method="POST">
            ...
            <input type="hidden" name="seat_price" value="<?php echo htmlspecialchars((string) $seat_price); ?>">
            ...
            <div class="price"><small style="display:block;font-size:14px;color:var(--muted);font-weight:400">Total Payable</small>Rs. <span id="total_price">0</span></div>
            <button type="submit" class="btn">Confirm Seats</button>
        </form>
    </div>
</div>
<script>
let selected=[];const price=<?php echo json_encode((float) $seat_price); ?>;
function toggleSeat(id,el){if(el.classList.contains('booked'))return;if(selected.includes(id)){selected=selected.filter(s=>s!==id);el.classList.remove('selected')}else{selected.push(id);el.classList.add('selected')}document.getElementById('seat_list').innerText=selected.length?selected.join(', '):'None';document.getElementById('total_price').innerText=(selected.length*price).toLocaleString('en-IN');document.getElementById('seat_input').value=selected.join(',')}
</script>
</body>
</html>
